...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        std::map<std::string, float> multi_devices;
        std::vector<std::string> devices, curr_devices;
        if (FLAGS_d == "ALL" || FLAGS_d.find("MULTI") != std::string::npos) {
            if (FLAGS_d == "ALL")
        ...
        std::string device_name = multi_devices.size() ? multi: FLAGS_d;
        ...
        int outHeight = 512, outWidth = 512, update = niter;
        bool stop = false;
        constexpr int FONT_FACE = cv::FONT_HERSHEY_SIMPLEX;
        constexpr double FONT_SCALE = 1;
        constexpr int THICKNESS = 2;
        cv::Mat res(outHeight, outWidth, CV_8UC3);
        std::vector<float> vec_fps;
        float max_fps = -FLT_MAX;
        int presenter_height = 150;

        cv::Size graphSize{outWidth, presenter_height};
        Presenter presenter("C", 10, graphSize);
        while ( !stop ) {
            ...
            if ((iteration % update) == 0) {
                ...
                vec_fps.push_back(fps);
                int height = presenter_height;
                res.setTo(cv::Scalar(0, 0, 0));
                presenter.drawGraphs(res);
                cv::putText(res, std::string("MAX FPS: ")+ std::to_string(max_fps), cv::Size{0, height+=50}, FONT_FACE, FONT_SCALE,
                            cv::Scalar{0, 0, 255}, THICKNESS);
                cv::putText(res, std::string("FPS: ")+ std::to_string(fps),
                        cv::Size{0, height+=50}, FONT_FACE, FONT_SCALE,
                            cv::Scalar{255, 0, 0}, THICKNESS);

                height+= 20;
                for (auto device : multi_devices) {
                    bool enabled = std::find(curr_devices.begin(), curr_devices.end(), device.first) != curr_devices.end();
                    cv::putText(res, device.first + " (stand-alone) FPS: " + std::to_string(device.second),
                                cv::Size{0, height += 50}, FONT_FACE, FONT_SCALE/2,
                                enabled ? cv::Scalar{0, 250, 0} : cv::Scalar{150, 150, 150},
                                THICKNESS);
                }

                cv::imshow("Priorities Test-bed", res);
                int key = cv::waitKey(1);
                const unsigned int index = key - 48 /*ASCII for 0*/;
                switch (key) {
                    case '0':
                    case '1':
                    case '2':
                    ...
                    case '4':
                    case '5':
                        if (multi_devices.size() && (index < multi_devices.size())) {
                            auto iter = std::next(multi_devices.begin(), index);
                            auto toggled_name = iter->first;
                            auto it = std::find(curr_devices.begin(), curr_devices.end(), toggled_name);
                            bool found = it != curr_devices.end();
                            if (found) {
                                if (curr_devices.size() != 1) {
                                    curr_devices.erase(it);
                                    std::cout << "Toggling " << toggled_name << "OFF" << std::endl;
                                }
                            } else {
                                curr_devices.insert(curr_devices.begin(), toggled_name);
                                std::cout << "Toggling " << toggled_name << "ON" << std::endl;
                            }
                            std::string m;
                            for (auto && device : curr_devices) {
                                m+= device;
                                m += ((device == curr_devices[curr_devices.size()-1]) ? "" : ",");
                            }
                            exeNetwork.SetConfig({{"MULTI_DEVICE_PRIORITIES", m}});
                            inferRequestsQueue.resetTimes();
                            iteration = 0;
                        }
                        break;
